...
import { Bookmark, Compass, Move3d } from "lucide-react"
import type { Property, ProcessingStatus } from "@/lib/types"

function StatusPill({ status, label }: { status: ProcessingStatus; label: string }) {
    ...
    <div className="glass flex flex-col overflow-hidden rounded-3xl">
      ...
      <div className="flex flex-1 flex-col gap-4 p-5">
        ...
        <div className="flex flex-wrap gap-2">
          <StatusPill status={property.tourStatus} label="Tour" />
          <StatusPill status={property.vastuStatus} label="Vastu" />
        </div>
        <div className="mt-auto flex gap-2 pt-1">
          <Link href={`/dashboard/property/${property.id}?tab=tour`} className="flex-1">
            <span className="flex items-center justify-center gap-1.5 rounded-lg border border-border py-2 text-sm hover:bg-accent">
              <Move3d className="size-4" /> View tour
            </span>
          </Link>
          <Link href={`/dashboard/property/${property.id}?tab=vastu`} className="flex-1">
            <span className="flex items-center justify-center gap-1.5 rounded-lg border border-border py-2 text-sm hover:bg-accent">
              <Compass className="size-4" /> Vastu
            </span>
          </Link>
        </div>
      </div>
